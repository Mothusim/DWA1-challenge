import { books, authors, genres, BOOKS_PER_PAGE } from "../DWA6/data.js";
...
class BookDetails extends HTMLElement {
    ...
    connectedCallback() {
      ...
      this.listItems.addEventListener('click', this.listItemClick.bind(this));

      this.shadowRoot.querySelector("[data-list-close]")
      .addEventListener("click", () => (this.shadowRoot.querySelector("[data-list-active]").open = false));
    }
  
    disconnectedCallback() {
        document.querySelector('[data-list-items]').removeEventListener('click', this.listItemClick.bind(this));
        
        this.shadowRoot.querySelector("[data-list-close]")
        .removeEventListener("click", () => (this.shadowRoot.querySelector("[data-list-active]").open = false));
    }
    ...
    listItemClick(event) {
      ...
      if (active) {
        ...
        this.shadowRoot.querySelector('[data-list-subtitle]').innerText = `${authors[active.author]} (${new Date(
          active.published
        ).getFullYear()})`;
